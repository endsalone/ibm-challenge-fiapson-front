<template>
	<main class="row">
		<div class="col-2">
			...
			<div class="container">
				<input type="text" class="input" v-model="pergunta" v-on:keyup.enter="enviarTexto"/>
				<button class="button ripple digitar" @click="abrirInput" v-if="!this.modoTexto">
					<i class="far fa-keyboard" style="font-size: 3rem;"></i>
				</button>
				<button class="button ripple digitar" @click="enviarTexto" v-else :disabled="disable">
					<img src="/image/loading.svg" width="64" v-if="loading && this.modoTexto"/>
					<i class="fas fa-play" style="font-size: 3rem;" v-else></i>
				</button>
				<button class="button ripple gravar" @click="gravar" v-if="!gravando">
					<img src="/image/loading.svg" width="64" v-if="loading && !this.modoTexto"/>
					<i class="fa fa-microphone" style="font-size: 3rem;" v-else></i>
				</button>
				<button class="button ripple gravar" @click="parar" v-else>
					<i class="fas fa-stop" style="font-size: 3rem;"></i>
				</button>
			</div>
			<div class="row">
				<h1 class="text-green" style="min-height: 50px;" id="resposta">{{this.resposta}}</h1>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		enviarTexto: function() {
			this.disable = true;
			this.loading = true;
			this.resposta = "Buscando conhecimento ...";
		...
		parar: function() {
			...
			this.axios
				.post("http://node.fiapson.com/audio", formData, config)
				.then(response => {
